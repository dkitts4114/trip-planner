...
from trip_planner.services import flight_search as svc
...
def test_flight_search_endpoint_happy_path(monkeypatch):
    ...
    def fake_search(req, *, searcher=None):  # noqa: ARG001
        return FlightSearchResult(
            outbound=[_to_itinerary(make_fli_result(price=149.0))],
            return_=None,
        )

    # Patch the symbol as imported by the api module
    ...
    r = client.post("/api/flights/search", json=body)
...
def test_flight_search_endpoint_returns_502_on_upstream_error(monkeypatch):
    def boom(req, *, searcher=None):  # noqa: ARG001
...
def test_flight_search_endpoint_surfaces_empty_warning(monkeypatch):
    def empty(req, *, searcher=None):  # noqa: ARG001
...
# silence "svc is unused" linter noise when re-importing below
_ = svc
